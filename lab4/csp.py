...
class CSP:
    ...
    def solve(self,
        ...
        while len(agenda) > 0:
            ...
            #check for step 5
            # if step == 5:
                # print "\nstate cur var: {}".format(state.get_current_variable_name())
            ...
            for value in values:
                ...
                children.append(Node(str(value), new_state)) # ??? How to get canceled when propagation comes in?

            cur_node.add_children(children)
            agenda = children + agenda # Create a DFS sequence
            step += 1
